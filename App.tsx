...
import { Step, type ChatMessage, type MissionData, type DialogueTurn, type FirstFeedbackData, type FinalFeedbackData, type AdvancedDialogueData, KeyExpression } from './types';
import { GeminiService } from './services/geminiService';
import { useSpeech } from './hooks/useSpeech';
import LoadingSpinner from './components/LoadingSpinner';
import { MicrophoneIcon, PlayIcon, RefreshIcon, CheckIcon } from './components/Icons';
...
const App: React.FC = () => {
    ...
    const startNewSession = useCallback(async () => {
        ...
        try {
            const data = await GeminiService.generateInitialMission(missionData?.missionTitle);
            setMissionData(data);
            setStep(Step.MISSION_PRESENTATION);
        } catch (error) {
            handleError(error);
        }
    }, [missionData?.missionTitle, addMessage, cancelSpeech]);
    ...
    useEffect(() => {
        ...
        const fetchFeedback = async () => {
            try {
                ...
                const data = await GeminiService.generateFirstFeedback(userRoleplayTranscript);
        ...
        fetchFeedback();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [step, missionData, userRoleplayTranscript, addMessage]);
    ...
    const handleAdvancedChallengeAccept = useCallback(() => {
        ...
        speak("Great! I'll show you the advanced scenario first.", async () => {
            try {
                addMessage('system', undefined, <LoadingSpinner />);
                const data = await GeminiService.generateAdvancedDialogue(missionData);
    ...
    useEffect(() => {
        ...
        const fetchFeedback = async () => {
            try {
                addMessage('system', undefined, <LoadingSpinner />);
                const data = await GeminiService.generateFinalFeedback(missionData, userRoleplayTranscript, userAdvancedRoleplayTranscript);
        ...
        fetchFeedback();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [step, missionData, userRoleplayTranscript, userAdvancedRoleplayTranscript, addMessage, speak]);
    ...
    const MissionCard = ({ mission }: { mission: MissionData }) => (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h2 className="text-2xl font-bold text-blue-600 mb-2">{mission.missionTitle}</h2>
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-100 max-w-3xl mx-auto shadow-2xl">
            <header className="bg-white shadow-md p-4 flex items-center justify-between z-10">
                <h1 className="text-2xl font-bold text-gray-800">Gina AI Tutor</h1>
            </header>
